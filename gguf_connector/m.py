...
def clone_file(url):
    try:
        ...
        with urllib.request.urlopen(url) as response, open(filename, 'wb'
            ) as file, tqdm(total=file_size, unit='B', unit_scale=True,
            unit_divisor=1024, desc=f'Downloading {filename}') as pbar:
            chunk_size = 1024
            while True:
        ...
        print(
            f"File cloned successfully and saved as '{filename}' in the current directory."
            )
...
def handle_user_input(data):
    while True:
        user_choice = input(
            f"Enter your choice (1 to {len(data)}) or 'q' to quit: ")
